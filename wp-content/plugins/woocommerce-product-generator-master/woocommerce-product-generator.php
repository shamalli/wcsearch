...
Vehicles';
	...
	public static function create_product() {
		...
		if ( !( $post_id instanceof WP_Error ) ) {
			...
			// rating
			update_post_meta($post_id, '_wc_average_rating', rand(10, 50)/10);
			...
			if (rand (1, 10) > 9) {
				//sale price
				$_price = $price*100;
				$sale_price = wc_format_decimal( floatval( rand( $_price-15, $_price ) / 100.0 ) );
				update_post_meta( $post_id, '_price', $sale_price );
				update_post_meta( $post_id, '_sale_price', $sale_price );
			}
			if (rand (1, 10) > 9) {
				// featured
				wp_set_object_terms($post_id, 'featured', 'product_visibility', true);
			}
			if (rand (1, 10) > 9) {
				// out of stock
			...
			foreach (self::$attributes AS $tax=>$attrs) {
				$terms = array();
				$attr_n = count( $attrs );
				$attr_max = rand( 1, 4 );
				$values = array();
				for ( $i = 0; $i < $attr_max ; $i++ ) {
					$term = $attrs[rand( 0, $attr_n - 1 )];
					$values[] = $term->name;
				}
				$data['raw_attributes'][] = array('name' => $tax, 'id' => $term->term_id, 'value' => $values);
			}
			
			$product = wc_get_product($post_id);
			
			// Set attributes function code /includes/admin/import/abstract-wc-product-importer.php
			$attributes          = array();
	...
	public static function get_image() {
		...
		if (isset($random_image[0])) {
			$image_id = $random_image[0];
			
			if ($image_id && wp_get_attachment_image_src($image_id)) {
				return array('id' => $image_id, 'file' => wp_get_attachment_image_src($image_id)[0]);
			}
		}
